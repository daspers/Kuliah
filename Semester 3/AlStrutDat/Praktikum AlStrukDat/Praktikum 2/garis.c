...
Deskripsi      : file body dari header garis.h
*/
...
{
	if((Absis(PAkhir(L1))==Absis(PAwal(L1)))&&(Absis(PAkhir(L2))==Absis(PAwal(L2))))
		return (Ordinat(PAkhir(L1))==Ordinat(PAwal(L1)))&&(Ordinat(PAkhir(L2))==Ordinat(PAwal(L2)));
	else if ((Absis(PAkhir(L1))!=Absis(PAwal(L1)))&&(Absis(PAkhir(L2))!=Absis(PAwal(L2))))
